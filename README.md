...
### Dockerでの実行方法
Dockerを使用してコンテナ環境で実行する場合、以下の手順に従ってください。
**注意:** Gmail APIの初回認証（ブラウザでの許可）はコンテナ内では困難なため、必ず一度ローカルでスクリプトを実行し、`config/gmailtoken.json` を生成してください。

1. **初回認証（ローカルでの実行）**
   上記「実行方法」の通り、一度手元で `python -m src.mailSystem` を実行し、ブラウザでGoogleアカウントのアクセスを許可します。これにより `config/gmailtoken.json` が作成されます。
2. **Docker Composeでの起動**
   トークンが作成されたら、以下のコマンドでコンテナをバックグラウンド起動します。
   ```bash
   docker-compose up -d
   ```
3. **ログの確認・停止**
   ...
   docker-compose down
   ```
コンテナ起動後は、ローカルと同様に定期的に自動記録が行われます。
